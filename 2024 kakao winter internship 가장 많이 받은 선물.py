...
def solution(friends, gifts):
    answer = 0
    # 준 선물 수, 받은 선물 수, 선물 지수
    give, take, pidx = {i: 0 for i in friends}, {i: 0 for i in friends}, {i: 0 for i in friends}
    willtake = {i: 0 for i in friends}
    for i in gifts:
        a, b = i.split(" ")
        give[a] += 1
        take[b] += 1
    for i in friends:
        pidx[i] = give[i] - take[i]

    for i in range(len(friends)):
        for j in range(len(friends)):
            if i != j:
                if friends[i] + " " + friends[j] in gifts:
                    if gifts.count(friends[i] + " " + friends[j]) > gifts.count(friends[j] + " " + friends[i]):
                        willtake[friends[i]] += 1
                    elif gifts.count(friends[i] + " " + friends[j]) < gifts.count(friends[j] + " " + friends[i]):
                        willtake[friends[j]] += 1
                    else:
                        if pidx[friends[i]] > pidx[friends[j]]:
                            willtake[friends[i]] += 1
                        elif pidx[friends[i]] < pidx[friends[j]]:
                            willtake[friends[j]] += 1
                else:
                    if pidx[friends[i]] > pidx[friends[j]]:
                        willtake[friends[i]] += 1
